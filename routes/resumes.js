...
router.post("/", authMiddleware, async (req, res) => {
  try {
    ...
    const { title, content, content2, content3, start, end, skills, email, phone } = req.body;
    const dev = await Resume.create({ userId, title, content, content2, content3, start, end, skills, email, phone });
...
router.put("/:resumeId", authMiddleware, async (req, res) => {
  try {
    ...
    if (userId !== existResum.userId) {
      ...
      await Resume.findByIdAndUpdate(resumeId, { $set: { title, content, content2, content3, start, end, skills, email, phone } });
    }
